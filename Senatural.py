...
def mostrar():
    ...
    st.title("📊 Dashboard - Green Life")
    def carregar_dados(arquivo): 
        try: 
            with open(arquivo, 'r',encoding ='utf-8', errors ='replace') as file:
    ...
    if not df.empty:
        ...
        graf_pizza = px.pie(df_categoria, names = "Categoria", values = "Total", title = "Participação no valor total por categoria", color_discrete_sequence = ["#483d8b","#18d2a9","#e61538","#b8da10"] )
        st.plotly_chart(graf_pizza)    
        
        graf_disp = px.scatter(df, x = "Valor", y = "Quantidade", color = "Categoria",
                                hover_data = ["Nome"], title = "Correlação entre Valor e Quantidade")
        ...
        graf_linha = px.line(df_data, x = "Data", y = "Total", markers = True, 
                            title = "Total vendido por data")
        
        st.plotly_chart(graf_linha)
        geolocalizacao = Nominatim(user_agent = "Dashboard - Green Life")
