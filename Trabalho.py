...
def mostrar():
    ...
    #Campo
    codigo_do_produto = st.text_input("Código do produto")
    ...
    categoria_produto= st.selectbox("Categoria do Produto",["Alimentos","Bebidas","Suplemento","Outros"])
    ...
    if st.button ("Adicionar produto"):
        if not codigo_do_produto or not nome_produto:
            st.warning("Preencha todos os campos obrigatorio")
        else:
            with open("produtos.txt","a") as arquivo:
                arquivo.write(f"Código: {codigo_do_produto}\n")
                ...
                arquivo.write(f"Data: {data_da_compra}\n")
